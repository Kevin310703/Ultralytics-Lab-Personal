...
class C2fGhost(nn.Module):
    """
    Drop-in replacement cho C2f:
    - cv1/cv2 dùng GhostConv (hoặc Conv nếu bạn muốn ổn định hơn)
    - m dùng GhostBottleneck thay Bottleneck
    """
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        super().__init__()
        # giữ logic hidden channels giống C2f
        self.c = int(c2 * e)
        self.cv1 = GhostConv(c1, 2 * self.c, 1, 1)      # có thể đổi sang Conv để “bảo thủ”
        self.cv2 = GhostConv((2 + n) * self.c, c2, 1, 1, act=True)

        # GhostBottleneck signature trong Ultralytics: (c1, c2, k=3, s=1)
        # Ở đây đặt c1=c2=self.c để giữ shape
...
class SimAM(nn.Module):
    def __init__(self, c1=None, e_lambda=1e-4): # c1 phải đứng đầu
        super().__init__()
